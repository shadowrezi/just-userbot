...
class BotControlApp:
    ...
    def __init__(self, master):
        ...
        ctk.CTkButton(
            master, text='Start', command=self.start_bot
        ).pack(pady=10)
        ctk.CTkButton(
            master, text='Stop', command=self.stop_bot
        ).pack(pady=10)
        ctk.CTkButton(
            master, text='Restart', command=self.restart_bot
        ).pack(pady=10)
        ctk.CTkButton(
            master, text='Quit', text_color='#ea0002', bg_color='#ffffff', command=self.quit_app
        ).pack(pady=10)
    ...
    def start_bot(self):
        ...
        self.bot = Process(target=userbot.main)
        self.bot.start()
